...
const NewConnectionDialog = React.memo(
  ({
    ...
    useEffect(() => {
      if (autoOpen) {
        setDialogTypesOpen(true);
      }
    }, [autoOpen]);

    const filteredPieces = pieces?.filter((piece) => {
      if (isNil(piece.auth)) return false;
      if (!piece.displayName.toLowerCase().includes(searchTerm.toLowerCase())) return false;
      if (allowedPieceNames && allowedPieceNames.length > 0) {
        return allowedPieceNames.includes(piece.name);
      }
      return true;
    });
    ...
    return (
      <>
        ...
        >
          ...
          <DialogContent
            className="min-w-[700px] max-w-[700px] h-[680px] max-h-[680px] flex flex-col"
