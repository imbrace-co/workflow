...
const createStepGraph: (
  ...
  return {
    ...
    edges:
      ...
      step.type !== FlowActionType.ROUTER && step.type !== FlowActionType.MULTIPLE_CHOICE
        ? [straightLineEdge]
        : [],
  };
};
  ...
  const skippedParents = flowStructureUtil
    ...
    .filter(
      (stepInPath) =>
        ...
        stepInPath.type === FlowActionType.MULTIPLE_CHOICE,
    )
    .filter((routerOrLoop) =>
      flowStructureUtil.isChildOf(routerOrLoop, stepName),
    )
    .filter((parent) => 'skip' in parent && parent.skip === true);

  return skippedParents.length > 0 || ('skip' in step && !!step.skip);
};
