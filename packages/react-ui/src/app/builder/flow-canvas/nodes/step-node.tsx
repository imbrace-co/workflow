...
const ApStepCanvasNode = React.memo(
  ({ data: { step } }: NodeProps & Omit<ApStepNode, 'position'>) => {
    ...
    return (
      ...
      >
        <div
          className="absolute left-full pl-3 text-muted-foreground text-sm flex items-center whitespace-nowrap opacity-0 transition-opacity duration-300 group-hover:opacity-100"
          style={{
            top: `${flowUtilConsts.AP_NODE_SIZE.STEP.height / 2 - 12}px`,
          }}
        >
          {step.name.replace(/^step_(\d+)$/, (_, num) => `Step ${num}`)}
        </div>
        <div
          className={cn(
                ...
                <div className="grow flex flex-col items-start justify-center min-w-0 w-full">
                  <div className=" flex items-center justify-between min-w-0 w-full">
                    ...
                    >
                      ...
                      {stepMetadata?.isEndPiece && (
                        <>
                          {console.log(`[EndFlowDebug] Rendering TERMINAL badge for step: ${step.name}`)}
                          <Badge
                            variant="outline"
                            className="text-[10px] h-4 px-1 ml-1 bg-red-100/50 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800"
                          >
                            TERMINAL
                          </Badge>
                        </>
