...
const StepSettingsContainer = () => {
  ...
  const form = useForm<FlowAction | FlowTrigger>({
    ...
    resolver: async (values, context, options) => {
      ...
      if (
        deepEqual(cleanedNewValues, cleanedCurrentValues) &&
        selectedStep.valid === valid
      ) {
        return result;
      }
            ...
            <ResizablePanel defaultSize={55} className="min-h-[80px]">
              <ScrollArea className="h-full">
                <div className="flex flex-col gap-4 px-4 pb-6">
                  ...
                  {(modifiedStep.type === FlowActionType.ROUTER || modifiedStep.type === FlowActionType.MULTIPLE_CHOICE) &&
                    modifiedStep && (
                      <RouterSettings readonly={readonly} isMultipleChoice={modifiedStep.type === FlowActionType.MULTIPLE_CHOICE}></RouterSettings>
                    )}
