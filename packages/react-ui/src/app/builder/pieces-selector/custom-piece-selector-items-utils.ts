...
export const createDocumentAIRouterStep = ({
  parentStepName,
  logoUrl,
  handleAddingOrUpdatingStep,
}: {
  ...
  return handleAddingOrUpdatingStep({
    pieceSelectorItem: {
      ...CORE_STEP_METADATA[FlowActionType.ROUTER],
      displayName: t('Check Document AI Status'),
    },
    operation: {
      type: FlowOperationType.ADD_ACTION,
      actionLocation: {
...
export const createMultipleChoiceRouterStep = ({
  parentStepName,
  logoUrl,
  handleAddingOrUpdatingStep,
}: {
...
export const createDocumentAIStep = ({
  pieceMetadata,
  operation,
  handleAddingOrUpdatingStep,
}: {
...
export const createMultipleChoiceStep = ({
  pieceMetadata,
  operation,
  handleAddingOrUpdatingStep,
}: {
  ...
  const actionName = MULTIPLE_CHOICE_ACTION;
  const createMultipleChoiceQuestionAction = getActionFromPieceMetadata(
    pieceMetadata,
    actionName,
  );
  if (isNil(createMultipleChoiceQuestionAction)) {
    return null;
  }
  return handleAddingOrUpdatingStep({
    pieceSelectorItem: {
      actionOrTrigger: createMultipleChoiceQuestionAction,
      type: FlowActionType.PIECE,
      pieceMetadata: pieceMetadata,
    },
    operation,
