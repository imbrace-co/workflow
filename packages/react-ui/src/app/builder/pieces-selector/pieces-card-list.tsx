...
import { AiAssistantsList } from './ai-assistants-list';
...
export const PiecesCardList: React.FC<PiecesCardListProps> = ({
  ...
  const allPiecesMetadata = categories.flatMap((c) => c.metadata);
  const isManageAiAssistantsSelected =
    selectedPieceMetadataInPieceSelector?.displayName === 'AI agents';
  ...
  if (selectedTab === PieceSelectorTabType.ALL) {
    return null;
  }
  return (
    <>
      ...
      {showActionsOrTriggersList && (
        <>
          ...
          {isManageAiAssistantsSelected ? (
            <AiAssistantsList
              operation={operation}
              allPiecesMetadata={allPiecesMetadata}
            />
          ) : (
...
const getItemHeight = (
  ...
  const isCoreAction =
    ...
    pieceMetadata.type === FlowActionType.MULTIPLE_CHOICE;
