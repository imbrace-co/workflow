...
import { createDocumentAIRouterStep, createDocumentAIStep, createMultipleChoiceRouterStep, createMultipleChoiceStep } from './custom-piece-selector-items-utils';
...
const convertStepMetadataToPieceSelectorItems = (
  ...
  switch (stepMetadataWithSuggestions.type) {
    ...
    case FlowActionType.MULTIPLE_CHOICE: {
...
> = ({
  ...
  operation,
    ...
    const setupMultipleChoicePiece = (item: PieceSelectorPieceItem) => {
      const multipleChoiceQuestionStepName = createMultipleChoiceStep({
        pieceMetadata: item.pieceMetadata,
        operation,
        handleAddingOrUpdatingStep,
      });
      if (!isNil(multipleChoiceQuestionStepName)) {
        createMultipleChoiceRouterStep({
          parentStepName: multipleChoiceQuestionStepName,
          logoUrl: item.pieceMetadata.logoUrl,
          handleAddingOrUpdatingStep,
        })
      }
    }

    const setupDocumentAIPiece = (item: PieceSelectorPieceItem) => {
      const documentAIStepName = createDocumentAIStep({
        pieceMetadata: item.pieceMetadata,
        operation,
        handleAddingOrUpdatingStep,
      });
      if (!isNil(documentAIStepName)) {
        createDocumentAIRouterStep({
          parentStepName: documentAIStepName,
          logoUrl: item.pieceMetadata.logoUrl,
          handleAddingOrUpdatingStep,
        })
      }
        ...
        <CardList className="min-w-[350px] h-full gap-0" listClassName="gap-0">
          {actionsOrTriggers &&
            actionsOrTriggers.map((item, index) => {
              ...
              const isMultipleChoiceAction =
                item.type === FlowActionType.PIECE &&
                item.actionOrTrigger.name === 'createMultipleChoiceQuestion';

              const isDocumentAIAction = item.type === FlowActionType.PIECE && item.actionOrTrigger.name === 'document_ai';

              // console.log("I am over here", isMultipleChoiceAction);
              // console.log("Document AI Action", isDocumentAIAction);
              // For multiple-choice action, we reuse the CreateTodoDialog component since it has similar UI/UX
              if (isMultipleChoiceAction) {
                // const multipleChoiceQuestionStepName = createMultipleChoiceStep({
                //   pieceMetadata: item.pieceMetadata,
                //   operation,
                //   handleAddingOrUpdatingStep,
                ...
                return (
                  <GenericActionOrTriggerItem
                    ...
                    stepMetadataWithSuggestions={item.pieceMetadata}
                    onClick={() => setupMultipleChoicePiece(item)}
                  ></GenericActionOrTriggerItem>
                )
              }

              if (isDocumentAIAction) {
                return (
                  <GenericActionOrTriggerItem
                    item={item}
                    hidePieceIconAndDescription={hidePieceIconAndDescription}
              ...
              if (isCreateTodoAction) {
                console.log("Open Dialog", hidePieceIconAndDescription);
                return (
