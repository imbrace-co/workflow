...
const FlowActionMenu: React.FC<FlowActionMenuProps> = ({
  ...
  const isChannelWorkflowFlow = (() => {
    if (!flow.folderId || !folders) return false;
    const folder = folders.find((f) => f.id === flow.folderId);
    return folder?.displayName === 'Channel Workflow';
  })();
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        {!embedState.hideFolders && !isChannelWorkflowFlow && (
          <PermissionNeededTooltip
            hasPermission={
        ...
        {/* Share Template dialog - commented out, replaced with copy link */}
        ...
        {!embedState.isEmbedded && (
          <DropdownMenuItem
            onClick={async () => {
              ...
              const token = authenticationSession.getImbraceToken() || '';
              const orgId = authenticationSession.getOrganizationId() || '';
              const shareUrl = `${baseUrl}/flows/${flow.id}?lang=${lang}&token=${token}&organizationId=${orgId}`;
              await navigator.clipboard.writeText(shareUrl);
              toast({ title: t('Flow link copied successfully') });
              setOpen(false);
            }}
          >
            <div className="flex cursor-pointer  flex-row gap-2 items-center">
