...
import { isFlattenedProperty } from './piece-property-utils';
...
export const formUtils = {
  ...
  removeUndefinedFromInput: (step: FlowAction | FlowTrigger) => {
    ...
    copiedStep.settings.input = Object.fromEntries(
      Object.entries(copiedStep.settings.input)
        .filter(([_, value]) => value !== undefined)
        .map(([key, value]) => {
          // Flatten special properties from DynamicProperties nesting:
          // { key: { key: "val" } } → "val"
          if (
            isFlattenedProperty(key) &&
            value !== null &&
            typeof value === 'object' &&
            !Array.isArray(value) &&
