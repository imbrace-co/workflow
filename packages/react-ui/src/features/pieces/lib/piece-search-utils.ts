...
const isFlowController = (stepMetadata: StepMetadata) => {
  ...
  return (
    ...
    stepMetadata.type === FlowActionType.ROUTER || stepMetadata.type === FlowActionType.MULTIPLE_CHOICE
  );
};
...
const isAppPiece = (metadata: StepMetadata) => {
  return (
    ...
    !isFlowController(metadata) &&
    (metadata.type === FlowActionType.PIECE && !metadata.categories.includes(PieceCategory.COMMUNICATION))
  );
};
...
const POPULAR_PIECES_NAMES = [
  ...
  'ask-a-question',
  'ai-connector',
  '@activepieces/piece-document-ai',
  '@activepieces/piece-send-message',
...
const sortByPieceNameOrder = (
  searchResult: PieceStepMetadataWithSuggestions[],
  orderNames: string[],
): PieceStepMetadataWithSuggestions[] => {
