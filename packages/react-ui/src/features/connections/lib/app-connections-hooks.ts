...
export const appConnectionsQueries = {
  ...
  useConnectionsOwners: () => {
    ...
    return useQuery({
      queryKey: ['app-connections-owners', organizationId],
      queryFn: async () => {
        // projectId removed - auto-resolved from organization context on backend
        ...
        let projectMembers: ProjectMemberWithUser[] = [];
