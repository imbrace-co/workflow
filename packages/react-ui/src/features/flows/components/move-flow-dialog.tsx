...
const MoveFlowDialog = ({ children, flows, onMoveTo }: MoveFlowDialogProps) => {
  ...
  const form = useForm<MoveFlowFormValue>({
    defaultValues: {
      folder: '',
  ...
  // Look up selected folder's display name
  const selectedFolderName =
    folders?.find((f) => f.id === watchFolder)?.displayName || '';
  const isAiCategory = isAiAgentCategory(selectedFolderName);
  ...
  const updateData = (field: keyof MoveFlowFormValue, value: any) => {
  ...
  >({
    mutationFn: async (values) => {
      // Build AI config if moving to AI category
      let aiConfig: { description: string; settings: { ai: any } } | undefined;

      if (isAiAgentCategory(selectedFolderName)) {
      ...
      const updatePromises = flows.map(async (flow) => {
        // 1. Change folder
        await flowsApi.update(flow.id, {
          type: FlowOperationType.CHANGE_FOLDER,
          request: { folderId: values.folder },
        });

        // 2. Update metadata (tags + AI config)
        const updatedMetadata = buildCategoryMetadata(
          selectedFolderName,
          flow.metadata,
          aiConfig,
        );
    ...
    onSuccess: () => {
      ...
      toast({ title: t('Moved flows successfully') });
    },
  });
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="moveFlowDirectResponse"
                            checked={field.value}
                            onCheckedChange={field.onChange}
                          />
                          <Label htmlFor="moveFlowDirectResponse">
            ...
            <div className="flex justify-end mt-4">
              <TooltipProvider>
                <Tooltip>
                  ...
                  {!isFormComplete && (
                    <TooltipContent>
                      <p>{t('Please complete all required fields')}</p>
                    </TooltipContent>
                  )}
                </Tooltip>
              </TooltipProvider>
