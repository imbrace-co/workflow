...
export const pieceHelper = {
    async executeProps( operation: ExecutePropsParams): Promise<ExecutePropsResult<PropertyType.DROPDOWN | PropertyType.MULTI_SELECT_DROPDOWN | PropertyType.DYNAMIC>> {
        ...
        const executionState = await testExecutionContext.stateFromFlowVersion({
            ...
            imbraceToken: operation.imbraceToken,
        })
        const property = await pieceLoader.getPropOrThrow({ pieceName: operation.pieceName, pieceVersion: operation.pieceVersion, actionOrTriggerName: operation.actionOrTriggerName, propertyName: operation.propertyName, devPieces: EngineConstants.DEV_PIECES, pieceSource: EngineConstants.PIECES_SOURCE })
    ...
    async executeValidateAuth(
        ...
        const piece = await pieceLoader.loadPieceOrThrow({ pieceName: piecePackage.pieceName, pieceVersion: piecePackage.pieceVersion, devPieces, pieceSource: EngineConstants.PIECES_SOURCE })
        const server = {
    ...
    async extractPieceMetadata({ devPieces, params }: { devPieces: string[], params: ExecuteExtractPieceMetadata }): Promise<PieceMetadata> {
        ...
        const piece = await pieceLoader.loadPieceOrThrow({ pieceName, pieceVersion, devPieces, pieceSource: EngineConstants.PIECES_SOURCE })
        const pieceAlias = pieceLoader.getPackageAlias({ pieceName, pieceVersion, devPieces })
        const pieceFolderPath = await pieceLoader.getPiecePath({ packageName: pieceAlias, devPieces, pieceSource: EngineConstants.PIECES_SOURCE })
