import { AppConnectionValue, assertNotNullOrUndefined, ExecutePropertyJobData, UserInteractionJobData, WorkerJobType } from '@activepieces/shared'
    ...
    async execute(jobData: UserInteractionJobData, engineToken: string, timeoutInSeconds: number): Promise<void> {
        ...
        switch (jobData.jobType) {
            ...
            case WorkerJobType.EXECUTE_PROPERTY: {
                const propJob = jobData as ExecutePropertyJobData & { imbraceToken?: string }
                response = await engineRunner(log).executeProp(engineToken, {
                    ...
                    timeoutInSeconds,
                    imbraceToken: propJob.imbraceToken,
                })
                break
            }
        }
