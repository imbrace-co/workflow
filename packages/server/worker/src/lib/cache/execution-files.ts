...
export const executionFiles = (log: FastifyBaseLogger) => ({
    async provision({
        ...
        // Skip npm installation for BUILTIN mode - pieces are pre-bundled
        ...
        if (piecesSource === 'BUILTIN') {
            log.info({
                ...
                mode: 'BUILTIN',
            }, 'Skipping piece installation - using pre-bundled builtin pieces')
        }
        else if (nonDevPieces.length > 0) {
            const startTime = performance.now()
