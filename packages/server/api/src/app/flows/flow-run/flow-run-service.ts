...
export const flowRunService = (log: FastifyBaseLogger) => ({
    ...
    async retry({ flowRunId, strategy, projectId }: RetryParams): Promise<FlowRun | null> {
        ...
        switch (strategy) {
            ...
            case FlowRetryStrategy.ON_LATEST_VERSION: {
                ...
                const { platformId: retryPlatformId, externalId: retryExternalId } = await projectService.getPlatformIdAndExternalId(oldFlowRun.projectId)
                return this.start({
                    flowId: oldFlowRun.flowId,
                    payload,
                    platformId: retryPlatformId,
                    orgId: retryExternalId,
    ...
    async resume({
        ...
        const { platformId, externalId: resumeExternalId } = await projectService.getPlatformIdAndExternalId(flowRun.projectId)
        if (matchRequestId || !checkRequestId) {
            return addToQueue({
                payload,
                flowRun,
                platformId,
                orgId: resumeExternalId,
    ...
    async test({ projectId, flowVersionId, parentRunId, stepNameToTest }: TestParams): Promise<FlowRun> {
        ...
        const { platformId, externalId } = await projectService.getPlatformIdAndExternalId(projectId)
        return addToQueue({
            flowRun,
    ...
    async handleSyncResumeFlow({ runId, payload, requestId }: { runId: string, payload: unknown, requestId: string }): Promise<EngineHttpResponse> {
        ...
        const { platformId: syncResumePlatformId, externalId: syncResumeExternalId } = await projectService.getPlatformIdAndExternalId(flowRun.projectId)
        await addToQueue({
            payload,
            flowRun,
            platformId: syncResumePlatformId,
            orgId: syncResumeExternalId,
            synchronousHandlerId,
