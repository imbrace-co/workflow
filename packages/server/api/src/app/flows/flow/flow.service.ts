...
export const flowService = (log: FastifyBaseLogger) => ({
    ...
    async list({
        ...
        if (operationStatus !== undefined) {
            queryBuilder.andWhere({ operationStatus: In(operationStatus) })
        }
        else {
            // By default, hide flows that are in the middle of being deleted
            queryBuilder.andWhere({ operationStatus: Not(FlowOperationStatus.DELETING) })
        }
        ...
        if (tags !== undefined && tags.length > 0) {
            // Filter flows that have the specified tags in their metadata
            // metadata.tags is an array of objects with id and name properties
            // We need to check if any of the tag names match the specified tags
            const tagConditions = tags
                .map((tag, index) => {
                    return `EXISTS (
                    SELECT 1 FROM jsonb_array_elements(ff.metadata->'tags') AS tag_elem
                    WHERE tag_elem->>'name' = :tag${index}
                )`
                })
                .join(' AND ')
            ...
            // Nếu metadata.tags không tồn tại → chỉ filter theo folderId (không áp tags)
            // Nếu metadata.tags tồn tại → áp cả folderId + tags
            queryBuilder.andWhere(
                `(ff.metadata IS NULL OR ff.metadata->'tags' IS NULL OR (${tagConditions}))`,
                tagParams,
            )
        }
    ...
    async count({ projectId, folderId, status, tags, operationStatus }: CountParams): Promise<number> {
        const queryBuilder = flowRepo().createQueryBuilder('ff')

        // Project scope
        queryBuilder.where({ projectId })
        ...
        // Optional status filter (single status)
        if (status !== undefined) {
            queryBuilder.andWhere({ status })
        }

        // Optional operationStatus filter; default excludes DELETING like list()
        if (operationStatus !== undefined) {
            queryBuilder.andWhere({ operationStatus: In(operationStatus) })
        }
        else {
            queryBuilder.andWhere({
                operationStatus: Not(FlowOperationStatus.DELETING),
            })
        }

        // Optional tags filter (same logic as list())
        if (tags !== undefined && tags.length > 0) {
            const tagConditions = tags
                .map((tag, index) => {
                    return `EXISTS (
