import { AppSystemProp, bindContext, exceptionHandler, PROXY_HEADER, REQUEST_ID_HEADER } from '@activepieces/server-shared'
...
async function setupBaseApp(): Promise<FastifyInstance> {
    ...
    const app = fastify({
        ...
        genReqId: (req) => {
            // Reuse an inbound x-request-id (from the gateway) so the trace is
            // continuous; otherwise mint one.
    ...
    // Bind the correlation context for each request (so the canonical logger
    // tags every line with request_id/ip/method/path/proxy) and echo the id back.
    app.addHook('onRequest', async (request, reply) => {
        const inboundProxy = request.headers[PROXY_HEADER]
        bindContext({
            requestId: request.id,
            ip: request.ip,
            method: request.method,
            path: request.url,
            proxy: typeof inboundProxy === 'string' ? inboundProxy : 'client',
            startTime: Date.now(),
        })
