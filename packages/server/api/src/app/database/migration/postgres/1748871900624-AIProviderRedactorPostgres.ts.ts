    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const provider of aiProviders) {
            try {
                ...
                log.error(`Failed to transform config for provider ${provider.id}: ${error instanceof Error ? error.message : String(error)}`)
                throw error
            }
        }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const provider of aiProviders) {
            try {
                ...
                log.error(`Failed to reverse transform config for provider ${provider.id}: ${error instanceof Error ? error.message : String(error)}`)
