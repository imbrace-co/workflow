...
export const projectService = {
    ...
    async getPlatformIdAndExternalId(projectId: ProjectId): Promise<{ platformId: string, externalId: string | undefined }> {
        const result = await projectRepo().createQueryBuilder('project').select(['"platformId"', '"externalId"']).where({
            id: projectId,
        }).getRawOne()
        const platformId = result?.platformId
        if (isNil(platformId)) {
            throw new Error(`Platform ID for project ${projectId} is undefined.`)
        }
        return { platformId, externalId: result?.externalId ?? undefined }
    },
