...
export const localPieceCache = (log: FastifyBaseLogger) => ({
    async setup(): Promise<void> {
        // Skip cache setup for BUILTIN mode - pieces come from registry
        const piecesSource = system.get(AppSystemProp.PIECES_SYNC_MODE)
        if (piecesSource === 'BUILTIN') {
            log.info('[localPieceCache] BUILTIN mode detected, skipping database piece cache setup')
            return
        }
    ...
    async getSortedbyNameAscThenVersionDesc(): Promise<PieceMetadataSchema[]> {
        // Return empty array for BUILTIN mode - pieces come from registry
        const piecesSource = system.get(AppSystemProp.PIECES_SYNC_MODE)
        if (piecesSource === 'BUILTIN') {
            return []
        }
