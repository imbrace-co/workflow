...
import { enterpriseFilteringUtils } from './piece-filtering-utils'
...
export const pieceMetadataService = (log: FastifyBaseLogger) => {
    ...
    return {
        ...
        async get({ projectId, platformId, version, name }: GetOrThrowParams): Promise<PieceMetadataModel | undefined> {
            ...
            console.log(`[PieceMetadataService] get(${name}, ${version}) found ${originalPieces.length} total pieces. Searching...`)
...
const loadBuiltinPiecesIfEnabled = async (_log: FastifyBaseLogger): Promise<PieceMetadataSchema[]> => {
    const piecesSource = system.get(AppSystemProp.PIECES_SYNC_MODE)
    if (piecesSource !== 'BUILTIN') {
        return []
    }

    try {
        // Look for the generated metadata JSON file
        // In Docker/Production: dist/packages/pieces/builtin/types/pieces.json
        // We are likely in dist/packages/server/api/main.js
        
        const possiblePaths = [
            path.resolve(process.cwd(), 'dist/packages/pieces/builtin/types/pieces.json'),
            path.resolve(__dirname, '../../../../../../../pieces/builtin/types/pieces.json'), // From source location
            path.resolve(__dirname, '../../../../pieces/builtin/types/pieces.json') // Fallback
        ]

        let metadataPath: string | null = null
        for (const p of possiblePaths) {
             // We need to use fs from imports. Check if fs is imported.
             // If not, we might need to add import. 
             // But let's check top of file imports.
        ...
        if (!metadataPath) {
             console.warn(`[PieceMetadataService] BUILTIN mode enabled but metadata file not found. Checked: ${possiblePaths.join(', ')}`)
             return []
        }

        const content = require('fs').readFileSync(metadataPath, 'utf-8')
        const metadata = JSON.parse(content)
        
        const mappedPieces = metadata.pieces.map((meta: any): PieceMetadataSchema => {
            return {
                id: apId(),
                name: meta.name,
        ...
        console.log(`[PieceMetadataService] Loaded ${mappedPieces.length} builtin pieces from ${metadataPath}`)
        return mappedPieces
    }
    catch (err) {  
        console.error(`[PieceMetadataService] Error loading builtin pieces metadata:`, err)
        return []
    }
}
...
async function findAllPiecesVersionsSortedByNameAscVersionDesc({
    ...
    const piecesFromBuiltin = await loadBuiltinPiecesIfEnabled(log)
    ...
    const allPieces = [...piecesFromBuiltin, ...piecesFromDevelopment, ...piecesFromDatabase]
