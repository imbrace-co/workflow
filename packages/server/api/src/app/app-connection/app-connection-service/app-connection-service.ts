...
export const appConnectionService = (log: FastifyBaseLogger) => ({
    ...
    async getOneOrThrowWithValue(params: GetOneParams): Promise<AppConnection> {
        const connectionById = await appConnectionsRepo().findOneBy({
            id: params.id,
            platformId: params.platformId,
            ...(params.projectId ? APArrayContains('projectIds', [params.projectId]) : {}),
        ...
        let decryptedConnection = await appConnectionHandler(log).decryptConnection(connectionById)

        // Auto-refresh expired OAuth2 tokens before returning
        const projectId = params.projectId ?? connectionById.projectIds?.[0]
        if (projectId && appConnectionHandler(log).needRefresh(decryptedConnection, log)) {
            const refreshedConnection = await appConnectionHandler(log).lockAndRefreshConnection({
                projectId,
                externalId: decryptedConnection.externalId,
                log,
            })
            if (!isNil(refreshedConnection)) {
                decryptedConnection = refreshedConnection
            }
        }

        const owner = isNil(decryptedConnection.ownerId) ? null : await userService.getMetaInformation({
            id: decryptedConnection.ownerId,
        })
        return {
            ...decryptedConnection,
            owner,
    ...
    async list({
        ...
        const queryBuilder = appConnectionsRepo()
            ...
            .where(querySelector)
        if (!isNil(imbraceUserId)) {
            queryBuilder.andWhere(
                `app_connection.metadata->>'imbraceUserId' = :imbraceUserId`,
                { imbraceUserId },
            )
        }
