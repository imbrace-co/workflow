    ...
    if (isObject(payload) && !isNil(principalProjectId)) {
        ...
        if ('projectId' in payload) {
            if (payload.projectId !== principalProjectId) {
                console.log('DENYING because of single entity' + payload.projectId + ' vs ' + principalProjectId)
                verdict = 'DENY'
            }
        }
            ...
            if (someEntityNotOwnedByCurrentProject) {
                console.log('DENYING because of multiple entities', someEntityNotOwnedByCurrentProject)
                verdict = 'DENY'
            }
        }
