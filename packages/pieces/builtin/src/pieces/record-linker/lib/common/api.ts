...
// --- imbrace microservice base URLs (legacy backend retired) ---
export const DATA_BOARD_BASE = (
  process.env['IMBRACE_DATABOARD_API'] || 'http://data-board.dev.imbrace.lan'
).replace(/\/$/, '');
...
export const LEGACY_APPS_BASE = (
  process.env['IMBRACE_LEGACY_APPS_API'] ||
  process.env['IMBRACE_PRIVATE_API'] ||
  ''
).replace(/\/$/, '');

const baseUri = DATA_BOARD_BASE;
...
export const callImbracePrivateApi = async (
  ...
  body?: any,
  headers: Record<string, string> = {}
) => imbraceApiRequestPrivateService(method, endpoint, body, headers);
