...
export const getAdSets = createAction({
  auth: facebookGraphAuth,
  name: 'get_ad_sets',
  displayName: 'Get Ad Sets',
  description: 'Get ad sets from a Facebook Ad Account',
  props: {
    ...
    limit: Property.Number({
      displayName: 'Limit',
      description: 'The maximum number of ad sets to retrieve',
      required: false,
      defaultValue: 25,
    }),
  ...
  async run(context) {
    try {
      ...
      const result = await facebookGraphCommon.getAdSets(accessToken, adAccountId, limit, fields);

      return result;
    } catch (error) {
      console.error('Error in get_ad_sets action:', error);
      throw error;
    }
  },
});
