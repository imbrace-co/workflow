...
export class KafkaClient implements IKafkaHelper {
	...
	constructor() {
		...
		this.kafka = new Kafka({
			...
			// Respect KAFKA_USE_SSL — the broker is plaintext (false). Forcing
			// SSL (ssl as an object) caused "socket disconnected before secure
			// TLS connection" on the assign publish (the End Message got lucky on
			// the bootstrap connection; the assign hit a leader whose SSL fails).
			ssl: this.kafkaSSL ? { rejectUnauthorized: rejectUnAuth } : false,
			// Only send SASL creds when SASL is enabled (empty creds otherwise).
			sasl: this.kafkaUseSASL === 'true'
				? {
						mechanism: 'plain',
